...
if free_space >= DESIRED_FREE_SPACE:
    ...
    gotify.create_message(f"Desired: {DESIRED_FREE_SPACE / GB:,.2f}\nFree space: {(free_space / GB):,.2f} Gb,\nNothing to do. 😊", title="TClean", priority=5)
else:
    gotify.create_message(f"Desired: {DESIRED_FREE_SPACE / GB:,.2f}\nFree space: {(free_space / GB):,.2f} Gb,\nSearching for torrents to delete...", title="TClean", priority=5)
    ...
    gotify.create_message(f"{len(sorted_candidates)} torrents eligible.", title="TClean", priority=5)
    for index in range(0, len(sorted_candidates)):
        ...
        gotify.create_message(f"Torrent Deleted:\n{sorted_candidates[index][1]['name']}\nFreed space: {sorted_candidates[index][1]['total_size'] / GB:,.2f} Gb",title="TClean",priority=5)
        ...
        if free_space >= DESIRED_FREE_SPACE:
            break
    gotify.create_message(f"TClean complete.\nFree space: {free_space:,.2f} Gb", title="TClean", priority=5)
